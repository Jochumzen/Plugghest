...
namespace Latex2MathML
{
    ...
    internal sealed class LatexParser
    {
        ...
        /// <summary>
        /// Check that Latex text has a \begin and an \end.
        /// </summary>
        private void AddDocument(LatexExpression Root)
        {
            List<LatexExpression> outline = Root.Expressions[0];
            ...
            for (int i = 0; i < outline.Count; i++)
            {
                if (outline[i].ExprType == ExpressionType.Command &&
                    ...
                    outline[i].Expressions[0][0].Name == "document")
                {
                    beginExists = true;
                }
                if (outline[i].ExprType == ExpressionType.Command &&
            ...
            for (int i = 0; i < outline.Count; i++)
            {
                outline[i].IndexInParentChild++;
            }
            LatexExpression beginDoc = new LatexExpression("begin", Root, 0, 0, Root.Customization);
            beginDoc.ExprType = ExpressionType.Command;
            beginDoc.Expressions[0].Add(new LatexExpression("document", beginDoc, 0, 0, Root.Customization));
            beginDoc.Expressions[0][0].Expressions = null;
            beginDoc.Expressions[0][0].ExprType = ExpressionType.PlainText;
            LatexExpression endDoc = new LatexExpression("end", Root, 0, 0, Root.Customization);
            endDoc.ExprType = ExpressionType.Command;
            endDoc.Expressions[0].Add(new LatexExpression("document", endDoc, 0, 0, Root.Customization));
            endDoc.Expressions[0][0].Expressions = null;
            endDoc.Expressions[0][0].ExprType = ExpressionType.PlainText;
            endDoc.IndexInParentChild = outline.Count + 1;
            outline.Insert(0, beginDoc);
            outline.Add(endDoc);
        }
